...
class G2PW(BertPreTrainedModel):
    def __init__(self, model_source, labels, chars, pos_tags,
        ...
        # self.bert = BertModel.from_pretrained("uer/albert-base-chinese-cluecorpussmall")
        # self.bert = AlbertModel.from_pretrained("uer/albert-base-chinese-cluecorpussmall")
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, phoneme_mask, char_ids, position_ids, pos_ids=None, label_ids=None, eps=1e-6):
        ...
        # print(f"ori_input_ids: {input_ids.shape}")
        input_ids = input_ids[0].unsqueeze(0)
        # print(f"after_input_ids: {input_ids.shape}")
        ...
        # print(f"orig_selected_hidden: {orig_selected_hidden.shape}")
        ...
        # print(f"after_use_con_phoneme_mask.shape: {phoneme_mask.shape}")
        logits = self.classifier(selected_hidden)
        # print(f"logits: {logits.shape}")
        # phoneme_mask = phoneme_mask[0]
        # phoneme_mask = phoneme_mask.unsqueeze(0)
